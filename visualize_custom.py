...
if __name__ == "__main__":
    ...
    folder_name = 'Deformed_custom_Images'
    ...
    deform_net1.load_state_dict(torch.load('deform_net_1.pth.tar', map_location=torch.device('cpu'))['state_dict'],
                                strict=False)

    deform_net2 = Deform_Net(in_features=128, out_features=(5 + 1) ** 3 * 3)
    deform_net2.load_state_dict(torch.load('deform_net_2.pth.tar', map_location=torch.device('cpu'))['state_dict'],
                                strict=False)

    classifier = Contrastive_PointNet()
    classifier.load_state_dict(torch.load('best_model.pth.tar', map_location=torch.device('cpu'))['state_dict'],
                               strict=False)
    ...
    with open(os.path.join(data_set_path + '/' + cls + '/train', file),'rb') as f:
        ...
        #point_mixup_custom
        ffd_mixup = ffd_2 = FFD([3, 3, 3])
        ffd_mixup.box_length = [2, 2, 2]
        # ffd_mixup.array_mu_z[1,2,0] =  -1.5
        ffd_mixup.array_mu_z[1, 0, 1] = 1.5
        ffd_mixup.array_mu_z[1, 2, 1] = 1.5

        # ffd_mixup.array_mu_z[1,0,2] =  0.5

        # ffd_mixup.array_mu_z[2,1,1] =  1
        # ffd_mixup.array_mu_z[0,1,1] =  1

        new_4 = ffd_mixup(np.asarray(norm_pointcloud) + 1)

        file_name = file.split('.')[0]
        img_path = folder_name + '/' + cls_folder_name + '/' + file_name
        # save img here
        pcwrite(img_path + '_origin', *(norm_pointcloud).T)
        pcwrite(img_path + '_deform1', *(new1).T)
        pcwrite(img_path + '_deform2', *(new2).T)
        pcwrite(img_path + '_mixup', *(new3).T)
        pcwrite(img_path + '_new_mixup', *(new_4).T)
        print("Done")
